...
import { BUILD_CONFIGURATION_PATH, PROJECT_ROOT_NODE_MODULES_PATH, PROJECT_ROOT_PATH } from "./webpack.constants";
...
function generateGlobalDependenciesAlias(): Record<string, string> {
  return Object.keys(packageConfig.dependencies).reduce(
    (acc: Record<string, string>, pkg: string) => ((acc[pkg] = path.resolve(PROJECT_ROOT_NODE_MODULES_PATH, pkg)), acc),
    {}
  );
}
